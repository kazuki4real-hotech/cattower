...
Cattower は、猫との写真・動画・言葉・ご飯・おもちゃを自分のために収蔵するプライベートな Web サービス。SNS ではなく「私設ミュージアム」を目指し、交流は任意の疑似散歩空間「猫町」に限定する。現在は設計フェーズで、リポジトリには設計文書のみが入っている。
...
- 推奨ダイヤル（Cattower の静かな性格に合わせ低め）: `DESIGN_VARIANCE 2–3` / `MOTION_INTENSITY 1–2` / `VISUAL_DENSITY 2–3`
