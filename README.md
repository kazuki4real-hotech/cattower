...
プロダクト設計と、主要 14 画面のフロントエンド基盤まで実装済みです。現在の画面はサンプルデータで動作し、認証、D1、R2、猫町のリアルタイム処理は後続フェーズで接続します。
...
Cloudflare への preview と deploy は、アカウント設定後に `apps/web` の `preview`、`deploy` script を使用します。
