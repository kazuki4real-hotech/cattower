...
投稿を他人に評価してもらうSNSではなく、猫との暮らしを自分のために記録するサービスです。記録は一つの共通フォームから作り、タグと任意のボードで整理します。

## Status

プロダクト設計と主要画面のフロントエンド基盤を実装し、`cattower-web` として Cloudflare Workers へデプロイ済みです。`main` への push で本番が自動更新されます。Better Auth、Google OAuth、本番 D1、非公開 R2、新規登録直後だけ表示する3 stepオンボーディング、途中再開、猫プロフィール、初期レスポンスから表示する複数猫切り替え、文章・写真・日付・タグ・複数猫を保存する共通記録エディター、自動下書き、編集・soft delete・restore、おうちの実データ表示、「去年の今ごろ」「3年前と今日」「今日の一枚」の実データ再発見、記録詳細、任意ボードの作成・名称変更・並び方変更・削除・記録追加・記録削除・手動並び替え、キーワード・日付・タグ・猫・メディアによる実データ検索、検索結果からのボード追加、投稿を急かさないempty state、家族招待、未読・既読を管理するWeb内通知、画像直接 upload、private画像配信、表示用WebP derivative、孤立・失敗画像の自動整理、runtime secrets まで設定済みです。Google OAuth callback後のD1 session、オンボーディング完了状態のredirect、再読み込み後のsession継続、認証済みブラウザからのR2直接uploadを本番確認済みです。認可境界はローカルのWorkers runtimeとD1 migrationを使う結合テストで継続検証します。両WorkerのWorkers Logsと非機密structured loggingも設定済みです。動画はhousehold単位の有料機能として提供する方針で、価格・上限・支払手段を決定するまでStreamは有効化していません。

お散歩機能は初期リリースの対象外です。Web の画面、ナビゲーション、設定、接続券発行 API は公開せず、Phase 6 全体をリリース後へ延期しています。

記録とボードは1日・7日・30日の期限付きURLで限定共有でき、取消・削除・account削除要求を公開閲覧へ即時反映します。共有URLはhashだけを保存し、共有画面からおうちの別resourceへ移動できません。

検索結果は50件単位のページ送りに対応し、URLに条件とページ番号を保持します。おうちの最近の記録にも、続きがある場合だけ検索への導線を表示します。

主要画面はreduced motion、キーボード操作、読み上げ可能な選択・開閉状態、動的なボード操作のlive regionに対応しています。補助テキスト色もアプリ背景上でWCAG 2.2 AAのcontrastを満たします。
...
初期リリースでは使わない realtime Worker の基盤を確認する場合は、別のterminalで実行します。Web アプリから接続券を発行する経路はありません。
...
```text
Build:  pnpm cf:build
Deploy: pnpm --filter @cattower/web run deploy
```
