...
プロフィール画像は選択時にJPEG/PNG/WebPと10MB上限を検査する。presign、R2への送信、server-side metadata/decode/derivative検査の失敗は同じ文言へまとめず、利用者が再選択・再試行できる理由を画面に表示する。表示には認証済みの`/api/media/:assetId?variant=profile`から512×512 WebPを使い、原本URLやR2 keyをブラウザへ公開しない。

skip は画面ごとに次のように扱う。

- 利用者の呼び名: 必須。Google表示名を初期値にし、未入力では次へ進めない
- 最初の猫の名前とテーマ色: 必須。最初の猫がない状態では完了扱いにしない
- 猫の写真: 任意。選ばずに登録でき、upload失敗後も「写真なしで完了へ」を選べる
- 完了画面: 入力項目を持たず、「おうちへ入る」または「最初の記録を作る」を選ぶ

画面送信ごとにcheckpointを保存するため、中断後は最後に完了した画面の次から再開する。`prefers-reduced-motion: reduce`では進行演出を静止させ、内容と操作順は変えない。
