...
- 背景はデザインガイドラインで定義済みの白系ニュートラル Snow と Polar を比較できる
- 既定背景は Polar。切り替えはクライアント側の一時表示状態とし、利用者情報や analytics へ保存しない
- 選択状態は色だけに依存せず、色名、Hex、枠、`aria-pressed` を併記する
- 背景切り替え以外の公開入口の役割、Google login、非公開既定の説明は変更しない
...
- 公開ランディングで Snow と Polar をキーボード操作で切り替えられ、選択状態を読み上げられる
- 透明背景のブランドビジュアルが Snow、Polar のどちらでも欠けずに表示される
