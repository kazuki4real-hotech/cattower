...
`/home` の「去年の今ごろ」と「3年前と今日」は、最近の記録の後に「あの頃の記録」という一つのセクションへまとめる。大きなカードを反復せず、各期間を罫線で分けた横長の記録行として表示する。写真がある場合は小さなサムネイル、文章だけの場合は静かなプレースホルダー、期間名・日付・タイトルまたは本文・猫名を表示し、記録詳細へリンクする。該当記録がない期間も同じ行を保ち、操作を要求しない短いempty stateを表示する。選択中の猫を切り替えたときは最近の記録と同じ猫へ揃える。
