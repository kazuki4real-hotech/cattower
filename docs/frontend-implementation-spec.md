...
`/home` の「去年の今ごろ」は最近の記録の後に一つの横長セクションとして置く。写真がある場合は小さなサムネイル、文章だけの場合は静かなプレースホルダー、日付・タイトルまたは本文・猫名を表示し、記録詳細へリンクする。該当記録がない場合も同じ見出しを保ち、操作を要求しない短いempty stateを表示する。選択中の猫を切り替えたときは最近の記録と同じ猫へ揃える。
