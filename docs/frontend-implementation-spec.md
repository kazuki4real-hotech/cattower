...
記録作成、おうちの最近の記録、記録詳細、オンボーディング、ボード本体のCRUDと記録操作は認証・D1・R2を含む縦切りを対象とする。検索、再発見、お散歩はサンプルデータから順次置き換える。
...
旧 `/onboarding/welcome` は `/onboarding/profile` へ一時 redirect する。旧 `/collections`、`/add`、`/town`、`/onboarding/preferences`、`/onboarding/theme` は対応する現行 route へ恒久 redirect する。

オンボーディング route は検索対象にせず、全ページで `robots: { index: false, follow: false }` を設定する。

`/home` の「去年の今ごろ」「3年前と今日」「今日の一枚」は、最近の記録の後に「あの頃の記録」という一つのセクションへまとめる。「今日の一枚」は日単位で選び直すランダム再発見の表示名とする。大きなカードを反復せず、各項目を罫線で分けた横長の記録行として表示する。写真がある場合は小さなサムネイル、文章だけの場合は静かなプレースホルダー、項目名・日付・タイトルまたは本文・猫名を表示し、記録詳細へリンクする。該当記録がない項目も同じ行を保ち、操作を要求しない短いempty stateを表示する。選択中の猫を切り替えたときは最近の記録と同じ猫へ揃える。
...
Material Symbols Rounded の使用名とGoogle Fontsのsubset指定は`apps/web/components/material-symbols.ts`を正本とし、`Icon`の`name`はその一覧に含まれる値だけを受け取る。新しいiconはcomponentで使う前にこの一覧へ追加し、ligature名が本文として露出しないことをtypecheckで保証する。Outlinedは記録buttonの`pets`だけを別subsetで読み込む。
...
- active cat selector は App Shell の Server Component が取得した猫一覧と選択値で初期描画し、hydration 後の `/api/cats` 取得を表示条件にしない
- `/settings` の「猫」タブで owner は猫の追加・プロフィール編集を行い、editor は閲覧だけ行う
- `/settings` は通常の本文幅を使う。1180px以下では設定分類を横タブの一列構成へ切り替え、1040px以下では猫一覧と編集フォームも一列へ切り替える
...
- `/record` は写真1枚、任意タイトル、本文、複数猫、記録日、最大10件のタグを一つのフォームで受け付け、本文または写真のどちらかを必須にする
- 写真は選択直後にpreviewし、送信・検査・記録保存の状態をフォーム内へ表示する。失敗時も入力済みの文章を保持して再試行できる
- `/record` は入力停止から約1秒後に下書きを自動保存し、再読込時は同じhousehold・作成者の下書きを初期レスポンスから復元する。保存中、保存済み、失敗と再試行をフォーム内に表示する
- `/entries/:entryId/edit` は作成と同じeditorを使い、ownerは全記録、editorは自分が作成した記録だけを編集できる。`version`不一致時は上書きせず再読込を案内する
- `/home` は選択中の猫に紐付く最新12件を初期レスポンスで取得し、最新の1件と最近の記録一覧をD1の実データで描画する
- 記録が0件の場合はサンプル投稿を表示せず、「今はこのままでよい」と伝えるempty stateと任意の「記録を残す」補助導線を一つだけ表示する。page headingには同じ投稿導線を重ねない
- `/entries/:entryId` はactive household内のreadyかつ未削除の記録だけを表示し、写真はprivate media endpointの記録用derivativeを使う
- 記録詳細は権限に応じて編集・削除を表示する。soft delete後は通常表示を墓標状態へ切り替え、ownerまたは作成者だけに復元操作を表示する

### Boards

- `/boards`はactive householdのボードを更新日時順に初期レスポンスで表示し、サンプル画像・サンプル件数を表示しない
- 0件では「必要になったときだけ作れる」empty stateを表示し、作成を強制する文言を使わない
- 作成フォームは名称と`manual` / `newest` / `oldest`の並び方を受け付け、保存中・重複・上限・一般エラーをフォーム内へ表示する
- ownerは全ボード、editorは自分が作成したボードにだけ編集・削除操作を表示する。削除は対象名を含む確認状態を経て実行する
- `/boards/:boardId`はボード名、件数、並び方と記録一覧を初期レスポンスで表示する。記録が0件でもsampleを使わず、ボードは空のまま利用できる
- 変更権限がある場合はactive householdのreadyかつ未削除の新しい記録を最大100件まで追加候補に表示し、追加済みは候補から除外する。100件より古い記録を探して追加する導線はP4-06の検索と接続する
- 記録の削除はボードからだけ外れ、元の記録は残ることを操作前後の文言で示す
- `manual`では各行に読み上げ可能な「上へ」「下へ」を表示し、先頭・末尾では無効化する。移動中と失敗をinline表示し、drag操作を必須にしない
- `newest` / `oldest`では日付による自動順を使い、手動移動操作を表示しない

### Search

- `/search`は認証後の初期レスポンスでactive householdの猫・tag候補と検索結果を取得し、sample dataやclient-sideの疑似filterを使わない
- キーワード、開始日、終了日、tag、猫、メディア種別、ページ番号をGET formとしてURL queryへ保存する。条件なしでは未削除の公開済み記録を新しい順に50件ずつ表示する
- キーワードはタイトル、本文、tagを対象にし、`%`、`_`、backslashは文字として検索できるようescapeする。日付は両端を含む
- 結果行はprivate media endpointのthumbnail、タイトルまたは本文、日付、猫、tagを表示し、記録詳細へ移動できる。画像がない記録も同じ情報階層で表示する
- 条件なしで記録が0件の場合は投稿を促さず、将来検索できることを説明して「おうちへ戻る」を表示する。条件ありで0件の場合だけ、任意形の説明と「条件をクリア」を表示する
- 変更可能なボードから遷移した場合は`boardId`を同じGET formで保持し、検索結果ごとに追加済み状態と「ボードへ追加」を表示する。追加は既存のboard versionとpolicyでguardし、成功後にserver stateを再取得する
- household内に記録がない状態と、条件に一致しない状態を区別する。invalidな日付範囲はフォーム直下へinline表示し、検索を実行しない
- 51件目以降は同じ条件とボード追加の文脈を保つ前後ページリンクで移動する。結果上部に表示範囲と全件数、最終ページ下部に「検索結果はここまでです」を表示する。存在しないページ番号は最後の有効ページへ戻す
- おうちの最近の記録は最大12件を表示し、古い記録がある場合だけ選択中の猫を保った「続きを見る」を検索へ接続する。続きがない場合は一覧の見終わりを短く示す
...
- App Shellのskip linkは`tabIndex="-1"`を持つ`main`へ移動する。mobile navigationの開閉buttonは対象navigationを`aria-controls`で参照し、`aria-expanded`で状態を伝える。縮小後は再表示button、展開後は最初のnavigation linkへfocusを移す
...
- reduced motionではCSS animation、transition、smooth scrollを即時表示へ切り替える
...
オンボーディングは共通の `OnboardingShell` 内で 3 枚のスライドと完了画面を表示する。
...
表示名と猫の名前は空のまま進めない。写真は任意であり、「スキップ」ではなく内容が分かる「あとで」を使う。
...
- 上部は左に小さな Cattower ロゴ、右に「1 / 3」。ラベル付きの大きなステップ一覧は置かない
...
3. 猫名保存時に `cats` を作成し checkpoint `2`
4. 写真を保存または「あとで」で checkpoint `3`
5. 完了画面の表示前に `onboarding_completed_at` を保存
6. 完了後に安全な `returnTo` があればそこへ、なければ `/home` へ移動
...
- progress は視覚表示に加えて「ステップ 2 / 3」の accessible name を持つ
- 色だけで選択、エラー、完了を示さない
...
- `onboarding_optional_skipped` (`step`: `photo`)
