...
公開ランディングの背景は Snow `--color-neutral-0` に固定する。Snow と Polar の比較用切り替えは検証を終えたため削除し、公開入口では利用者設定として扱わない。
...
- ブランド表示は透明背景の `apps/web/public/images/cattower-wordmark.webp` を正本とし、ミント色の立体文字、濃色の縁、やわらかなツヤを保つ。アプリ左上、mobile header、公開ランディング、オンボーディングで同じ画像を使い、ダミーの `T` アイコンとテキスト表記は使わない
...
### 7.5 記録editorの状態と破壊操作（決定）

- autosaveは入力欄と同じcard内に「保存中」「保存済み」「失敗」を高さが大きく変わらないinline状態として表示する
- autosave失敗時は文章を保持したまま「下書きを再保存」を表示し、toastだけに依存しない
- soft deleteは即時実行せず、同じ場所に説明と確定・取消ボタンを展開する。復元可能であることを削除前に伝える
- 編集、削除、復元は権限がある場合だけ記録詳細の補助領域へ表示し、本文と写真より強く見せない

### 7.6 ボード内の記録操作（決定）

- ボード詳細は写真カードを重ねず、記録の小さなthumbnail、名称、日付を罫線付きの一列listで表示する
- 手動並び替えはdragだけに依存せず、各記録の「上へ」「下へ」をicon buttonと読み上げlabelで提供する
- ボードから外す操作は元の記録が残ることを同じ行で示し、確定前に取消できる状態を挟む
- 追加・削除・移動の通信中は関連する操作を無効化し、失敗はlistの直前へinline表示する。順序はserver保存成功後にだけ画面へ反映する
- 空のボードにはsample記録を置かず、記録が元の場所に残ったまままとめられることを説明する

### 7.7 検索画面（決定）

- 検索条件は常時見える一つのformへまとめ、keywordを主列、日付・tag・猫・メディアを補助列として配置する。条件をpillの羅列やmodalへ隠さない
- 検索結果は写真カードのgridにせず、thumbnail、本文、日付、猫、tagを罫線listで比較できるようにする
- ボードから遷移した検索では対象ボード名と戻り先をフォーム前に表示し、各結果の追加操作と追加済み状態を同じ行に置く
- 検索中の画面置換animationは使わず、server responseで結果を確定する。空状態は「記録がない」と「条件に合わない」を文言と操作で区別する
- 50件を超える結果は件数表示と「前の50件」「次の50件」だけの静かなページ送りをlistの下へ置く。ページ番号の帯や無限スクロールは使わない
- 最終ページでは「検索結果はここまでです」と明示する。おうちの最近の記録も、続きがある場合だけ検索への「続きを見る」を出し、ない場合は見終わりを短く示す
- desktopでは条件を複数列、mobileでは一列へ畳み、入力値、label、focus ringを保つ

### 7.8 おうちの再発見（決定）

- 「去年の今ごろ」「3年前と今日」「今日の一枚」は別々の大きなカードにせず、「あの頃の記録」という一つの領域へまとめる
- 各項目は罫線で分けた記録行とし、項目名、thumbnailまたは文章用placeholder、日付、記録名、猫名の順で表示する
- 該当記録がない項目も行を省略せず、同じ位置に投稿操作を伴わない短いempty stateを表示する
- mobileでは項目名を行の先頭へ独立させ、thumbnail、本文、詳細への矢印を次の行へ収める。自動animationは追加しない

### 7.9 Empty state（決定）

- データがない状態を失敗や未完了として扱わず、現在の状態を短く説明して「今はこのままでよい」と伝える
- 「最初の」「始めよう」「追加してみましょう」のように行動を急かす文言を使わない。次の操作は命令形を避け、必要な場合だけ補助buttonとして一つ置く
