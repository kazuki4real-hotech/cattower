...
- 「去年の今ごろ」と「3年前と今日」は別々の大きなカードにせず、「あの頃の記録」という一つの領域へまとめる
- 各期間は罫線で分けた記録行とし、期間名、thumbnailまたは文章用placeholder、日付、記録名、猫名の順で表示する
- 該当記録がない期間も行を省略せず、同じ位置に投稿操作を伴わない短いempty stateを表示する
- mobileでは期間名を行の先頭へ独立させ、thumbnail、本文、詳細への矢印を次の行へ収める。自動animationは追加しない
