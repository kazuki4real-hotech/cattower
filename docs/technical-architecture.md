...
- `expiresAt`、`revokedAt`、`resourceType`、`resourceId` を持ち、平文URLは作成responseだけで返す
- ownerはhousehold内の全resource、editorは既存policyで変更可能な自作resourceだけにshareを作成できる。取消はownerまたはshare作成者に限定する
- response に `Cache-Control: private, no-store`
- `X-Robots-Tag: noindex, nofollow, noarchive`
- share page はApp Shellと内部linkを描画せず、token scopeの記録・ボード項目・entry derivativeだけを表示する
- 記録・ボード削除、householdのaccount削除要求、share revoke、expiryをrequestごとに再確認する。ボード内の削除記録は共有範囲から除外する
- share作成は利用者ごとに1時間10件、公開ページとtoken scope mediaはtokenと接続元の組み合わせごとに1分60requestへ制限する。接続元はraw IPを保存せず、token hash・時間窓と合わせたSHA-256 keyだけを最大2分保持する
- `lastAccessedAt`は1時間bucketの運用値とし、閲覧者の履歴や公開指標に使わない
...
- share閲覧の短期rate limit keyは期限付きD1行としてbounded cleanupし、IPやfingerprintを恒久保存しない
...
- share作成・取消、hash storage、expiry、resource/account削除失効、token scope media、作成・閲覧rate limit
...
共有期限通知は通知summary/list request時に、本人が作成した有効なshareのうち24時間以内に期限切れとなる最大20件を確認して作る。dedupe keyにはshare IDを使うがpayloadへtoken、resource ID、本文を複製せず、通知自体もshare expiryで失効する。
