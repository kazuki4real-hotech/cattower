...
- Current boundary: フロントエンドの route と client interaction は sample data で動作する。認証、D1、R2、Stream、Durable Objects はまだ production data path に接続していない
...
- environment ごとに R2 CORS を設定し、許可 origin、method、header を最小化する
...
- 開発・staging の支出上限を Stream resource 作成前に決める
...
- Web は Workers Builds で `main` から自動デプロイする。D1 migration を導入するまでは build と deploy のみを実行する
...
7. R2 presigned PUT の署名、secret rotation、environment 別 CORS、upload 後の binding 検証
