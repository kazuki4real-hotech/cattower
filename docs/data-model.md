...
R2画像の`provider_key`は原本の`{asset prefix}/original`を指す。プロフィール表示用derivativeは同じasset prefixの`profile-512.webp`、記録表示用は`entry-1600.webp`へ保存し、別の`media_assets` rowは作らない。assetを削除するときは原本と既知のderivativeを同じ削除状態遷移で消す。
...
- 「3年前と今日」も同じindexと日付正規化を使い、3年前同日の前後3日に候補を限定する。再発見の年数追加で専用tableや重複indexは作らない
- ランダム再発見はready・未削除の対象件数と、`entries(household_id, occurred_at, deleted_at)`に沿う安定順のoffset取得で選ぶ。日ごとの選択結果は保存せず、専用table、乱数列、追加indexを作らない
