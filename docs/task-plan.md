...
- Updated: 2026-07-14
- Progress rule: 実装と同じ pull request で checkbox と判断を更新する
...
- [x] P0-02 私室・展示室・猫町の体験モデルを定義する
- [x] P0-03 猫町の presence と anti-SNS 制約を定義する
- [x] P0-04 Cloudflare を前提に技術構成を決定する
- [x] P0-05 論理データモデルを作成する
- [x] P0-06 MVP acceptance と実装フェーズを作成する
- [ ] P0-07 正式名称と独自ドメインを決定する
- [ ] P0-08 猫町のビジュアル方式を決定する
- [ ] P0-09 提供プランの画像容量・動画分数と Stream の開発・運用予算を決定する

Exit criteria: P0-07〜08 は実装開始を妨げないが public beta 前に決定する。P0-09 のうち開発・staging の Stream 支出上限は P1-10/P1-18 より前、利用者向け動画上限は P3-15 より前に決定する。

## Phase 1 — Foundation and technical spikes

### Repository

- [ ] P1-01 pnpm workspace を初期化する
- [ ] P1-02 `apps/web` に Next.js App Router + TypeScript strict を構築する
- [ ] P1-03 OpenNext Cloudflare adapter と `wrangler.jsonc` を設定する
- [ ] P1-04 `apps/realtime` に Worker + Durable Object skeleton を作る
- [ ] P1-05 `packages/db`、`packages/domain`、`packages/ui` を作る
- [ ] P1-06 ESLint、formatter、typecheck、test scripts を統一する
- [ ] P1-07 GitHub Actions で lint/typecheck/unit/build を実行する

### Cloudflare resources

- [ ] P1-08 local/staging 用 D1 を作成する
- [ ] P1-09 local/staging 用 private R2 bucket を作成する
- [ ] P1-10 staging Stream binding と private playback を設定する
- [ ] P1-11 realtime Durable Object binding と migration を設定する
- [ ] P1-12 `wrangler types` により binding 型を生成する
- [ ] P1-13 observability と非機密 structured logging を設定する

### Spikes

- [ ] P1-14 OpenNext + D1 の read/write smoke test
- [ ] P1-15 Better Auth + Google OAuth + D1 session の smoke test
- [ ] P1-16 bucket 限定 R2 access key、Worker 内 SigV4 署名、environment 別 CORS を含む browser direct upload の smoke test
- [ ] P1-17 R2 binding で upload 後 metadata を確認し、Images Binding `.info()` と derivative 生成、入力上限、EXIF、animated/巨大画像を検証する
- [ ] P1-18 支出上限を設定後、Stream Direct Creator Upload で MOV を upload/playback し保存・配信分数を計測する
- [ ] P1-19 別 Worker 発行の短命 ticket で WebSocket 接続する
- [ ] P1-20 Durable Object hibernation 後に接続・状態が復元することを検証する
- [ ] P1-21 FTS5 trigram の日本語検索、1〜2 文字 LIKE fallback、index size、write/query cost を検証する

Exit criteria: staging URL 上で auth、D1、image、video、WebSocket の縦切りデモが動き、選定上の blocker がない。

## Phase 2 — Identity, household and cat profile

- [ ] P2-01 Better Auth schema と migration を確定する
- [ ] P2-02 Google login/logout/session refresh を実装する
- [ ] P2-03 onboarding と user preferences を実装する
- [ ] P2-04 初回 household 自動作成、owner 一つ・editor 複数所属、active household 切り替えを実装する
...
- [ ] P2-09 profile image upload と private delivery を実装する
...
- [ ] P3-01 entry、entry_cats、template detail schema/migration を作る
- [ ] P3-02 共通 entry editor と autosaved draft を実装する
- [ ] P3-03 何気ない瞬間テンプレート
- [ ] P3-04 今日のひとことテンプレート
- [ ] P3-05 好きなもの図鑑テンプレート
- [ ] P3-06 ご飯メモテンプレート
- [ ] P3-07 はじめて・記念日テンプレート
- [ ] P3-08 くらべるテンプレート
- [ ] P3-09 複数猫への紐付けを実装する
...
Exit criteria: 6 テンプレートが写真・動画・文章を失わず保存でき、再読込後も正しく表示・編集できる。

## Phase 4 — Private museum experience

- [ ] P4-01 おうち画面の収蔵棚レイアウト
- [ ] P4-02 system collection の自動分類
- [ ] P4-03 custom collection CRUD
- [ ] P4-04 collection item の手動並び替え
- [ ] P4-05 entry detail の図録風レイアウト
- [ ] P4-06 検索と template/date/media filters
- [ ] P4-07 去年の今ごろ
- [ ] P4-08 3 年前と今日
- [ ] P4-09 最近登場していないおもちゃ
- [ ] P4-10 このご飯を初めて食べた日
...
- [ ] P6-23 opt-in 時だけ猫町の日次まとめ通知を生成する
