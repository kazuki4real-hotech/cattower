...
- [ ] P1-05 `packages/db`、`packages/domain`、`packages/ui` を作る（`db` / `domain` は完了、`ui` は未着手）
...
- [x] P1-09 local/production 共用 private R2 `cattower-media-production` と localhost CORS を設定する
- [ ] P1-10 production Stream binding と private playback を設定する
...
- [ ] P1-16 bucket 限定 R2 access key、Worker 内 SigV4 署名、localhost/production CORS を含む browser direct upload の smoke test（API、SigV4、metadata/decode 検査、localhost CORS は実装済み。token と production origin を登録して E2E を行う）
...
- [ ] P2-03 登録後 onboarding（利用者名、最初の猫、暮らし方、完了演出）と user preferences を実装する（D1 永続化は完了。Google OAuth E2E 後に完了扱い）
...
- [ ] P2-09 profile image upload と private delivery を実装する（presign/完了検査/private delivery は実装済み。R2 token と production CORS 登録後に完了扱い）
