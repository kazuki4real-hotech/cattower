...
- [x] P2-03UI onboarding 3 step の一列UI、前後移動、進捗、完了画面を実装する
- [x] P2-03A onboarding の途中保存、再開、任意写真の「あとで」、reduced-motion を実装する
...
- [x] P2-05 猫の作成・編集を実装し、テーマ色・保管を持たない構成へ簡略化する
- [x] P2-06 複数猫切り替え UI を実装する
- [x] P2-06A 猫切り替えを初期レスポンスで描画し、設定画面のPC幅と初期表示のレイアウト移動を修正する
...
- [x] P2-12 家族招待・承認と upload 完了・失敗の Web 内通知を実装する
...
- [x] P4-03 board CRUD（記録追加・削除・並び替えはP4-04）
- [x] P4-04 board item の追加・削除・手動並び替え
- [x] P4-05 entry detail レイアウト
- [x] P4-06 検索と tag/date/media/cat filters、ボード追加導線
- [x] P4-07 去年の今ごろ
- [x] P4-08 3 年前と今日
- [x] P4-11 ランダムな一枚
- [x] P4-12 pagination と明確な「見終わり」を実装する
- [x] P4-13 empty state を投稿圧力のない文言で整える
- [x] P4-14 reduced motion、keyboard、screen reader QA
- [x] P4-15 Material Symbolsの使用名とfont subsetを同期し、ligature名の露出を防ぐ

Exit criteria: タイムラインを使わずに、新旧の記録をタグ・ボード・検索・再発見から辿れる。

## Phase 5 — Limited sharing

- [x] P5-01 cryptographic share token と hash storage
- [x] P5-02 entry share page
- [x] P5-03 board share page
- [x] P5-04 expiry/revoke UI
- [x] P5-05 noindex/no-store/security headers
- [x] P5-06 share page から scope 外 resource へ遷移できない test
- [x] P5-07 deleted entry/account で即時失効する test
- [x] P5-08 share endpoint rate limit
- [x] P5-09 share expiry の Web 内通知を実装する

Exit criteria: 選択した範囲だけを期限付きで閲覧でき、取消しが即時反映される。

## Phase 6 — お散歩 MVP（初期リリース後へ延期）

Decision 2026-07-18: 初期リリースから除外する。初期リリースではナビゲーション、`/walk` 画面、お散歩設定、接続券発行 API を公開せず、本 Phase の未完了項目はリリース後に再評価する。
