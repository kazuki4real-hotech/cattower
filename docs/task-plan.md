...
- [ ] P1-15 Better Auth + Google OAuth + D1 session の smoke test（runtime secretsとOAuth開始は本番確認済み。callback後のsessionを認証済みブラウザで確認する）
- [ ] P1-16 bucket 限定 R2 access key、Worker 内 SigV4 署名、localhost/production CORS を含む browser direct upload の smoke test（API、SigV4、token、metadata/decode検査、CORSは実装済み。認証済みブラウザでE2Eを行う）
...
- [ ] P2-09 profile image upload と private delivery を実装する（presign/完了検査/private delivery、R2 token、production CORSは実装済み。認証済みE2E後に完了扱い）
