...
| --- | --- | --- | --- |
| ペット SNS | ドコノコ | 動物ごとのブック、写真投稿、フォロー、ハート、コメント、迷子支援 | 公開交流が中心。文章や持ち物を自分だけのために収蔵する体験ではない |
...
- Cattower は私室を中心にし、交流は猫町へ明示的に持ち出した一枚だけ
- Cattower は写真がない言葉、ご飯、おもちゃ、比較記録を同列に扱う
...
- 一日単位や一枚単位を強制しない
- 時系列だけでなく、収蔵棚とテーマ別コレクションで整理する
- テキストだけの記録と「物の履歴」を主要機能にする
...
- 全生活情報を万能管理するのではなく、記憶の収蔵と再発見へ焦点を絞る
- 部屋をゲームの dashboard としてではなく、展示図録のような静かな navigation として使う
- 猫町によって、猫の飼い主に不足しやすい偶然の接点を補う
...
| Instagram | Cattower |
| --- | --- |
| 人に見せるために投稿する | 自分が忘れないために収蔵する |
| 写真・動画が主役 | 写真、動画、文章、物、出来事が同列 |
| 新しい投稿と推薦が強い | 古い記録を見つけ直す |
| likes、views、followers | popularity count を置かない |
| algorithm が次を選ぶ | 飼い主が作った棚を歩く |
| public profile が交流の入口 | private room が入口 |
...
2. 写真だけでなく、言葉・おもちゃ・ご飯を template で収蔵
3. timeline ではなく、部屋と棚による navigation
4. 古い記録を自然に再発見する仕組み
5. 記録を一枚だけ持ち出す、非同期的な疑似散歩「猫町」
...
Mitigation: 「写真日記」ではなく、物と言葉を含む私設ミュージアムとして onboarding と home を設計する。
...
Mitigation: streak、未投稿通知、入力必須欄を最小化する。「何も持たず猫町へ行く」ことも許可する。
...
- 写真以外の template が自然に使われるか
- 30 日以上前の記録を見返す行動が生まれるか
- 棚 navigation が timeline より理解しやすいか
- 猫町で「反応しなければ」という圧力を感じないか
- resting/trace 表現を不正確または不気味だと感じないか
- private と town card の境界を誤解しないか
- 利用者が猫町を使わなくてもサービス価値を感じるか
