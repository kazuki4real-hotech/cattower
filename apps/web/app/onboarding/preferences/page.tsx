...
export default async function PreferencesPage() { const viewer = await getViewer(); if (!viewer) redirect("/onboarding/welcome"); const snapshot = await getOnboardingSnapshot(viewer.db, viewer.session.user.id, viewer.household.id); return <OnboardingShell current={2}><p className="eyebrow">残したい時間</p><h1>最初のあしあとを、選んでください。</h1><p className="lede">選ばなかった棚も、あとから自由に追加できます。</p><OnboardingPreferencesForm initial={snapshot.memoryPreferences} /></OnboardingShell>; }
