...
export async function POST(request: Request) {
  ...
  if (!body || typeof body.catId !== "string") return Response.json({ error: "invalid_request" }, { status: 400 });
  const validated = validateImageUpload({ contentType: body.contentType, byteSize: body.byteSize, fileName: body.fileName });
  if (!validated.ok) return Response.json({ error: validated.code }, { status: 400 });

  const cat = await viewer.db.query.cats.findFirst({
    where: and(eq(cats.id, body.catId), eq(cats.householdId, viewer.household.id)),
  });
  if (!cat) return Response.json({ error: "cat_not_found" }, { status: 404 });
  const membership = await requireActiveMembership(viewer.db, viewer.session.user.id, cat.householdId);
  if (!membership) return Response.json({ error: "forbidden" }, { status: 403 });

  if (!viewer.env.R2_ACCESS_KEY_ID || !viewer.env.R2_SECRET_ACCESS_KEY) {
    return Response.json({ error: "upload_signing_not_configured" }, { status: 503 });
  }
  ...
  const signed = await signer.sign(uploadUrl, { method: "PUT", headers, aws: { signQuery: true } });
  ...
  return Response.json({ assetId, uploadUrl: signed.url, method: "PUT", headers, expiresIn: UPLOAD_EXPIRY_SECONDS });
}
